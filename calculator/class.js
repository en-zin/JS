...
const draw = (numberJson) => {
    ...
    let sevenSegmentTable = sevenSegment.sevenSegmentTable;
    ...
    for(const sevenSegmentNmber of numberJson) {    //配列の中身分for分で回して7セグを点灯させる
        sevenSegment.drowSevenSegment(...sevenSegmentTable[sevenSegmentNmber],(x)); //x軸を回すごとに20ずらす
        x = x + 20;
    };
    kigou();
};
...
const start = ()  => {          
    ...
    new SevenSegment().drowSevenSegment(...new SevenSegment().sevenSegmentTable[0],0);
};
